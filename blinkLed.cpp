...
int getIntervalForValue(int value, int uncritical, int critical)
{
  ...
  {
    double parameter = (double)(value - uncritical)/(critical - uncritical);
    return (int)(biggestInterval - parameter * (biggestInterval - smallestInterval));
  }
  else
  {
...
{
  int co2Interval = getIntervalForValue(co2Value, uncriticalCO2value, criticalCO2value);;
  int tvocInterval = getIntervalForValue(tvocValue, uncriticalTVOCvalue, criticalTVOCvalue);
